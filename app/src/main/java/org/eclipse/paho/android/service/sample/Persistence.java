...
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
  ...
  /** device name **/
  public static final String COLUMN_NAME = "name";// this is device ID to SUB/PUB

  /** Table column for host **/
  public static final String COLUMN_HOST = "host";
  /** Table column for client id **/
  public static final String COLUMN_client_ID = "clientID";// this is device name, just to show
  ...
  public void persistConnection(Connection connection) throws PersistenceException {
    ...
    values.put(COLUMN_NAME, ClientConnections.d_ID);
  ...
  public List<Connection> restoreConnections(Context context) throws PersistenceException
  {
    ...
    for (int i = 0; i < c.getCount(); i++) {
      ...
      //----------------------------------------------------------------------------------------> listing device to SUB/PUB
      name.add( c.getString(c.getColumnIndexOrThrow(COLUMN_NAME)) );// for PUB/SUB
      ID.add( c.getString(c.getColumnIndexOrThrow(COLUMN_client_ID)) );// just for show
