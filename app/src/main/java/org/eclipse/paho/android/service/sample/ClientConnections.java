...
public class ClientConnections extends ListActivity {
  ...
  //=================================================
  public static boolean doAdd = true;
  public static boolean doDel = false;
  public static String clientHandle =null;
  public static String client_ID_del = "";
  public static boolean doSub = true;
  public static boolean doCon = true;
  public static String d_ID = "";// need to insert [device_ID] to database
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    Map<String, Connection> connections = Connections.getInstance(this)
        .getConnections();// get from database
  ...
  protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    connectAction(dataBundle);// ------------------------------------------------------> try connect to server
   ...
   */
  private void connectAction(Bundle data) { //----------------------------------------> result from NewConnection
    ...
    // The basic client informationisDisconnected
    ...
    Connections.getInstance(this).addConnection(connection);//------------------------->add connection to database
    if (doConnect) {
      try {
        client.connect(conOpt, null, callback);// ------------------------------------->try connect Client to Servre
      }
  ...
  private class LongClickItemListener implements OnItemLongClickListener, ActionMode.Callback, OnClickListener {
    ...
    public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
      ...
      //Connection c_del = arrayAdapter.getItem(position);
      //clientHandle = c_del.handle();
    ...
    private void delete()
    {
      ...
      if (connection.isConnectedOrConnecting()) {
        ...
        for(int i=0; i < Persistence.ID.size(); i++) {
          if (Persistence.name.get(i).equals(connection.getId()) ) {
            client_ID_del = Persistence.ID.get(i);
            break;
          }
        }
        */
